...
def main():
    ...
    np.set_printoptions(
        precision=3
    )  # 这是为了让输出的结果更精确，set_printoptions()的作用是设置输出的精度
    ...
    for i in range(1, len(ob_loss)):
        observed_loss = np.concatenate(
            (observed_loss, ob_loss[i].reshape(n, 1)), axis=1
        )
    ...
    # 存储的时候加上节点信息
    plt.savefig(
        "non_linear/beta_{}_epoch{}_reweight_observation_n{}_d{}_s{}_{}_{}.png".format(
            beta, epoch, n, d, s0, graph_type, sem_type
        )
    )
    ...
    # 保存在文件夹20nodes中
    plt.savefig(
        "non_linear/beta_{}_epoch{}_reweight_totalloss_n{}_d{}_s{}_{}_{}.png".format(
            beta, epoch, n, d, s0, graph_type, sem_type
        )
    )
