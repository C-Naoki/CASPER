...
if __name__ == "__main__":
    ...
    for key, item in ys_ori[i].items():
        ...
        plt.fill_between(
            xs_ori[i], means_ori - stds_ori, means_ori + stds_ori, alpha=0.05, color="b"
        )

        plt.plot(xs_casper[i], means, label=f"{method_new}")
        plt.fill_between(
            xs_casper[i], means - stds, means + stds, alpha=0.05, color="r"
        )
    ...
    plt.savefig(
        os.path.join("figs_nonlinear/", gf + setting + linearty + "batch_final.png"),
        bbox_inches="tight",
    )
    plt.cla()
