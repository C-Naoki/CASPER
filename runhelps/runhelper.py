...
def config_parser():
    ...
    parser.add_argument(
        "--modeltype",
        default="notears",
        choices=["notears", "golem", "daggnn", "grandag"],
    )
    parser.add_argument("--s0", default=40, type=int)
    parser.add_argument(
        "--d", default=20, type=int
    )  # 如果是10 下面的n就是1000, 如果是20就是2000, 如果d>50, n最好加大
    parser.add_argument("--n", default=2000, type=int)
    parser.add_argument(
        "--linear_sem_type",
        default="gauss",
        choices=["gauss", "exp", "gumbel", "uniform", "logistic", "poisson", "laplace"],
    )
    parser.add_argument(
        "--sem_type", default="gp", choices=["gp-add", "mlp", "gp", "mim"]
    )
    parser.add_argument("--graph_type", default="ER", choices=["SF", "ER", "BA"])
    parser.add_argument(
        "--linear", action="store_true", help="whether to use linear synthetic data"
    )
    ...
    parser.add_argument(
        ...
        choices=["real", "synthetic", "testing", "sachs_full"],
    )
    ...
    parser.add_argument(
        "--reweight_epoch", type=int, default=0, help="the epoch begin to reweight"
    )

    parser.add_argument(
        "--temperature", type=int, default=20, help="softmax_tmperature"
    )
    parser.add_argument(
        "--adaptive_epoch",
        default=10,
        type=int,
        help="number of iterations for adaptive reweight",
    )
    parser.add_argument(
        "--adaptive_lr",
        default=0.0001,
        type=float,
        help="learning rate for adaptive reweight",
    )
    parser.add_argument(
        "--adaptive_lambda",
        default=0.001,
        type=float,
        help="adaptive lambda for l1 regularization",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help=" 0: reweight + batch ; 1: baseline; 2: batch + baseline; 3: wasserstein",
    )
    parser.add_argument(
        "--iter_mod", type=int, default=5, help="the iteration mod for wassertein"
    )
    parser.add_argument(
        "--clip", type=float, default=0.01, help="the clip for Lipshitz"
    )
    parser.add_argument(
        "--space_lr",
        type=int,
        default=0.001,
        help="the learning rate of space optimizer",
    )

    parser.add_argument(
        "--scaled_noise", action="store_true", help="noise scaling varying samples"
    )
    return parser
