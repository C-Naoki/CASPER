...
def notears_linear(
    ...
    def _loss(W):
        ...
        M = (
            X @ W
        )  # M是X矩阵和W矩阵的乘积，X是样本矩阵，W是权重矩阵，X的维度是[n, d]，W的维度是[d, d]，M的维度是[n, d]
        if loss_type == "l2":
            R = X - M
    ...
    bnds = [
        ...
        for i in range(d)
        for j in range(d)
    ]  # bnds是约束条件，每个元素的第一个元素是下界，第二个元素是上界
    if loss_type == "l2":
    ...
    for iter in tqdm.tqdm(range(max_iter)):
        ...
        alpha += rho * h  # 更新alpha
        if (
            h <= h_tol or rho >= rho_max
        ):  # 如果拉格朗日函数的值小于等于h_tol或者步长大于等于rho_max，则结束循环
            break
